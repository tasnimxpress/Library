...
class JournalClasses{
		...
		public function insertJournalIntoDB($post,$files){
			...
			if (!in_array($FfileType, $Fextentions) && $Fsize > 200000) {
                    ...
                    $query = "INSERT INTO tbl_journal(title,type,author,description,image,status,files) VALUES('$title','$category','$author','$description','$uploadPath',0,'$uploadPath2')";

                    $result = $this->db->insert($query);
                    if ($result && $didUpload && $result != false) {
                        $successNote = "<span style='color:green;text-align: center;display: block;'>Journal inserted successfully </span>";

						return $successNote;
					}
					else{

						$fieldError = "<span style='color:red;text-align: center;display: block;'>Journal couldn't be inserted!!</span>";
        ...
        public function getJournal(){
            ...
            $query = "SELECT * FROM tbl_journal WHERE status=0 ORDER BY id";
            ...
            if ($result && mysqli_num_rows($result)>0) {
                ...
                // $fieldError = "<span style='color:red;text-align: center;display: block;'>Journal couldn't be Found!!</span>";
        ...
        public function editJournalIntoDB($id){

            $query = "SELECT * FROM tbl_journal WHERE id=$id AND status=0";
        ...
        public function updateJournalIntoDB($post,$files,$id){
            ...
            if ($fileName=='' && $Fname=='') {
                $query = "UPDATE tbl_journal SET	title ='$title',
			            							type='$category',
			            							author ='$author',
                ...
                if ($result) {
                    echo "<script>window.location.href = 'journallist.php';</script>";
                }
                else{
                ...
                if (!in_array($fileType, $fileExtensions) && $fileSize > 200000) {
                    ...
                    $query = "SELECT * FROM tbl_journal WHERE id='$id'";
                    $result = $this->db->select($query);
        ...
        public function deleteJournalIntoDB($id){

            $query = "DELETE FROM tbl_journal WHERE id=$id";
