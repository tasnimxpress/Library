...
class ReportClasses{
		...
		public function insertReportIntoDB($post,$files){
			...
			if (!in_array($FfileType, $Fextentions) && $Fsize > 200000) {
                    ...
                    $query = "INSERT INTO tbl_report(title,type,author,description,image,status,files) VALUES('$title','$category','$author','$description','$uploadPath',0,'$uploadPath2')";

                    $result = $this->db->insert($query);
                    if ($result && $didUpload && $result != false) {
                        $successNote = "<span style='color:green;text-align: center;display: block;'>Report inserted successfully </span>";

						return $successNote;
					}
					else{

						$fieldError = "<span style='color:red;text-align: center;display: block;'>Report couldn't be inserted!!</span>";
        ...
        public function getReport(){
            ...
            $query = "SELECT * FROM tbl_report WHERE status=0 ORDER BY id";
            ...
            if ($result && mysqli_num_rows($result)>0) {
                ...
                // $fieldError = "<span style='color:red;text-align: center;display: block;'>Report couldn't be Found!!</span>";
        ...
        public function editReportIntoDB($id){

            $query = "SELECT * FROM tbl_report WHERE id=$id AND status=0";
        ...
        public function updateReportIntoDB($post,$files,$id){
            ...
            if ($fileName=='' && $Fname=='') {
                $query = "UPDATE tbl_report SET	title ='$title',
			            							type='$category',
			            							author ='$author',
                ...
                if (!in_array($fileType, $fileExtensions) && $fileSize > 200000) {
                    ...
                    $query = "SELECT * FROM tbl_report WHERE id='$id'";
                    $result = $this->db->select($query);
        ...
        public function deleteReportIntoDB($id){
