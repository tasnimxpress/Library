...
class VideoClasses{
		...
		public function insertVideoIntoDB($post,$files){
            ...
            $Fextentions = ['MP4','mp4','WMV','wmv']; // Get all the file extensions
			...
			if (!in_array($FfileType, $Fextentions) && $Fsize > 31457280) {
                    ...
                    $query = "INSERT INTO tbl_video(title,type,author,description,image,status,files) VALUES('$title','$category','$author','$description','$uploadPath',0,'$uploadPath2')";

                    $result = $this->db->insert($query);
                    if ($result && $didUpload && $result != false) {
                        $successNote = "<span style='color:green;text-align: center;display: block;'>Video inserted successfully </span>";

						return $successNote;
					}
					else{

						$fieldError = "<span style='color:red;text-align: center;display: block;'>Video couldn't be inserted!!</span>";
        ...
        public function getVideo(){
            ...
            $query = "SELECT * FROM tbl_video WHERE status=0 ORDER BY id";
            ...
            if ($result && mysqli_num_rows($result)>0) {
                ...
                // $fieldError = "<span style='color:red;text-align: center;display: block;'>Video couldn't be Found!!</span>";
        ...
        public function editVideoIntoDB($id){

            $query = "SELECT * FROM tbl_video WHERE id=$id AND status=0";
        ...
        public function updateVideoIntoDB($post,$files,$id){
            ...
            if ($fileName=='' && $Fname=='') {
                $query = "UPDATE tbl_video SET	title ='$title',
			            							type='$category',
			            							author ='$author',
                ...
                if (!in_array($fileType, $fileExtensions) && $fileSize > 31457280) {
        ...
        public function deleteVideoIntoDB($id){
