...
<?php
  ...
  if ($_SERVER['REQUEST_METHOD']=="GET" && isset($_GET['id'])) {
    ...
    if ($_GET['id']!=null) {
        $editResult = $bk->editJournalIntoDB($_GET['id']);
    } else {
  ...
  if ($_SERVER['REQUEST_METHOD']=="POST" && isset($_POST['editBook'])) {
    ...
    if ($_GET['id']!=null) {
        $updateBookResult = $bk->updateJournalIntoDB($_POST,$_FILES,$_GET['id']);
    } else {
        echo "<script>window.location.href = '404.php';</script>";
    }
  }
        ...
        <div class="container-fluid dashboard">
          ...
          <form action="" method="POST" enctype="multipart/form-data">
            <div class="row formpart">
              ...
              <input type="text" class="form-control formclass" id="title" name="category" value="<?= isset($book['type']) ? $book['type'] : 'Journal'; ?>" required>
              </div>
            </div>
            <div class="row formpart">
